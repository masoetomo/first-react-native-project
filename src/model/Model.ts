...
export default class Model {
  ...
  _createTables(transaction: SQLite.Transaction) {
    // DANGER! For dev only
    const dropAllTables = false;
    if (dropAllTables) {
      transaction.executeSql('DROP TABLE IF EXISTS Siswa;');
  ...
  /*
    ...
    _preVersion1Inserts(transaction: SQLite.Transaction) {
    ...
    _preVersion2Inserts(transaction: SQLite.Transaction) {
